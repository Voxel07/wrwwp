...
foreach ( get_users() as $u ) {
	$curr_role = 'mitglied';
	if ( in_array( 'vorstand', (array) $u->roles, true ) ) {
		$curr_role = 'vorstand';
	} elseif ( in_array( 'frischling', (array) $u->roles, true ) ) {
		$curr_role = 'frischling';
	}
...
foreach ( get_users( array( 'role' => 'mitglied' ) ) as $wrw_m ) {
	$mitglieder_list[] = array(
		'id'          => $wrw_m->ID,
		'displayName' => esc_html( $wrw_m->display_name ),
	);
}
