...
if (in_array('vorstand', (array) $user->roles))      $role_label = 'Vorstand';
elseif (in_array('mitglied', (array) $user->roles))  $role_label = 'Mitglied';
elseif (in_array('frischling', (array) $user->roles)) $role_label = 'Frischling';
...
if ($events_query->have_posts()) {
    while ($events_query->have_posts()) {
        ...
        if (is_array($parts) && in_array($user_id, $parts)) {
            $year = get_the_time('Y');
            if (!isset($attended[$year])) $attended[$year] = array();
            $attended[$year][] = esc_html(get_the_title());
        }
    }
    wp_reset_postdata();
}
...
$wrw_wp_data['profileData'] = array(
    ...
    'updatedParam' => isset($_GET['updated']) && $_GET['updated'] === '1',
