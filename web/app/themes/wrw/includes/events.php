<?php
/**
 * Events Custom Post Type and Handlers.
 *
 * @package wrw
 */
...
function wrw_handle_event_registration() {
	...
	if ( ! wp_verify_nonce( sanitize_key( wp_unslash( $_POST['wrw_event_nonce'] ) ), 'register_event_' . intval( wp_unslash( $_POST['event_id'] ) ) ) ) {
		wp_die( 'Security check failed.' );
	}

	$event_id = isset( $_POST['event_id'] ) ? intval( wp_unslash( $_POST['event_id'] ) ) : 0;
	$user_id  = get_current_user_id();
	$action   = isset( $_POST['registration_action'] ) ? sanitize_text_field( wp_unslash( $_POST['registration_action'] ) ) : ''; // Action: register or unregister.
	...
	if ( 'register' === $action && ! in_array( $user_id, $participants, true ) ) {
		$participants[] = $user_id;
	} elseif ( 'unregister' === $action && in_array( $user_id, $participants, true ) ) {
...
function wrw_handle_event_creation() {
	...
	if ( ! isset( $_POST['wrw_event_nonce'] ) || ! wp_verify_nonce( sanitize_key( wp_unslash( $_POST['wrw_event_nonce'] ) ), 'create_event' ) ) {
		wp_die( 'Security check failed.' );
	}

	$title    = isset( $_POST['event_name'] ) ? sanitize_text_field( wp_unslash( $_POST['event_name'] ) ) : '';
	$content  = isset( $_POST['event_description'] ) ? wp_kses_post( wp_unslash( $_POST['event_description'] ) ) : '';
