...
function wrw_save_user_profile_fields( $user_id ) {
	...
	if ( isset( $_POST['wrw_notification_pref'] ) ) {
		update_user_meta( $user_id, 'wrw_notification_pref', sanitize_text_field( wp_unslash( $_POST['wrw_notification_pref'] ) ) );
	}
	if ( isset( $_POST['wrw_ribbon'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing
		update_user_meta( $user_id, 'wrw_ribbon', sanitize_text_field( wp_unslash( $_POST['wrw_ribbon'] ) ) );
	}
	if ( isset( $_POST['wrw_phrase'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing
		update_user_meta( $user_id, 'wrw_phrase', sanitize_text_field( wp_unslash( $_POST['wrw_phrase'] ) ) );
	}
	if ( isset( $_POST['wrw_joined_date'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing
		update_user_meta( $user_id, 'wrw_joined_date', sanitize_text_field( wp_unslash( $_POST['wrw_joined_date'] ) ) );
	}
	if ( isset( $_POST['wrw_birthday'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing
		update_user_meta( $user_id, 'wrw_birthday', sanitize_text_field( wp_unslash( $_POST['wrw_birthday'] ) ) );
	}
