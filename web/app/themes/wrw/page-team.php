...
foreach ( $users as $user ) {
	if ( $user->ID <= 1 ) {
		continue; // Optional skip admin if needed.
	}
	$matched_role = 'mitglied'; // fallback.
	foreach ( $user->roles as $wrw_role ) {
		if ( array_key_exists( $wrw_role, $grouped_members ) ) {
			$matched_role = $wrw_role;
			break;
		}
	}
	if ( array_key_exists( $matched_role, $grouped_members ) ) {
		...
		if ( $birthday ) {
			try {
				...
				unset( $e );
				// Silently fail if date format is invalid.
			}
		}
		...
		if ( $months > 0 ) {
			if ( $diff->y >= 1 ) {
				$duration_str = "Dabei seit {$diff->y} Jahr" . ( $diff->y > 1 ? 'en' : '' ) . ( $diff->m > 0 ? " und {$diff->m} Monat" . ( $diff->m > 1 ? 'en' : '' ) : '' );
			} else {
				$duration_str = "Dabei seit {$diff->m} Monat" . ( $diff->m > 1 ? 'en' : '' );
			}
		}
		...
		foreach ( $all_events as $evt ) {
			...
			if ( is_array( $parts ) && in_array( $user->ID, $parts, true ) ) {
				++$visited;
			}
		}
		...
		if ( $mentor_id ) {
			...
			if ( $mentor_obj ) {
				$mentor_name = rtrim( esc_html( $mentor_obj->display_name ) );
			}
		}
